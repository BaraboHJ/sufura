...
class Dish
{
    ...
    public static function create(PDO $pdo, int $orgId, int $actorUserId, array $payload): array
    {
        ...
        $stmt = $pdo->prepare(
            "INSERT INTO dishes (org_id, name, {$categoryColumn}, description, yield_servings, active, created_at, updated_at)
             VALUES (:org_id, :name, :category_id, :description, :yield_servings, :active, NOW(), NOW())"
        );
    ...
    public static function categoryColumn(PDO $pdo): string
    {
        ...
        foreach (['dish_category_id', 'category_id'] as $column) {
            if (self::dishesTableHasColumn($pdo, $column)) {
